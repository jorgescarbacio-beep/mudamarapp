...
function BurgerKingPage({ tarifas, saveTarifas, updateBalance }) {
  ...
  const collectionName = 'viajes_burgerking';

  useEffect(() => {
    const defaultTarifas = { burgerKingViaje: 20000 };
  ...
  const ivaTotal = subTotal * IVA_RATE;
  const totalGeneral = subTotal + ivaTotal;

  const TARIFA_FIELDS = [{ key: 'burgerKingViaje', label: 'Tarifa por Viaje' }];
  ...
  const handleAddTrip = async (e) => {
    ...
    const price = localTarifas.burgerKingViaje || 0;
  ...
  const downloadRemitoPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text(`Remito de Servicios - Burger King`, 14, 22);
    ...
    doc.save(`Remito_BurgerKing_${new Date().toISOString().slice(0,10)}.pdf`);
  };
  ...
  return (
    <div>
      <EditTarifasModal
        ...
        title="Editar Tarifa Burger King"
      ...
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-4xl font-bold text-white">Gestión: Burger King</h1>
